// Get your shorts on - this is an array workout!
    ...
    // 2. Give us an array of the inventors' first and last names
      const result2 = inventors.map(i => `${i.last}, ${i.first}`);
      console.log(result2);

    // Array.prototype.sort()
    // 3. Sort the inventors by birthdate, oldest to youngest
      const result3 = inventors.sort((i1, i2) => i1.year > i2.year? 1 : -1 );
    ...
    // var boulevards = document.querySelectorAll(".mw-category-group a");
    // var filtered = Array.from(boulevards);
    // var result = filtered.map(e => e.textContent);
    // console.log(result.filter(e => e.indexOf('d') > -1));
    // console.log(result.filter(e => e.includes('d')));
    ...
    // Sum up the instances of each of these
      ...
      const result8 = data.reduce((obj, vehicle) => {
        obj[vehicle] = obj[vehicle] ? ++obj[vehicle] : 1;
        return obj;
      }, {});
      console.log(result8);
